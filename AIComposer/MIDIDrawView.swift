...
class MIDIDrawView: NSView {
    ...
    override func drawRect(dirtyRect: NSRect) {
        ...
        let noteColor = NSColor.blueColor()
    ...
    private func createMIDINoteBezier(note: MusicNote) -> NSBezierPath {
        let notePath = NSBezierPath(roundedRect: NSRect(x: Double(note.timeStamp) * 60.0 + 15.0, y: (Double(note.midiNoteMess.note) * 2.0) - 90.0, width: Double(note.midiNoteMess.duration) * 60.0, height: 3.5), xRadius: 1.0, yRadius: 1.0)
        return notePath
    }
